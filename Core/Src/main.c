...
int main(void)
{
  ...
  // erase and write test pattern
  for (var = 0; var < SECTORS_COUNT; var++) {
      if (CSP_QSPI_EraseSector(var * MEMORY_SECTOR_SIZE, (var + 1) * MEMORY_SECTOR_SIZE - 1) != HAL_OK) {
          Error_Handler();
      }
      if (CSP_QSPI_WriteMemory(buffer_write, var * MEMORY_SECTOR_SIZE, sizeof(buffer_write)) != HAL_OK) {
    	  Error_Handler();
      }
  }

  // verify using memory mapped mode
  if (CSP_QSPI_EnableMemoryMappedMode() != HAL_OK) {
      Error_Handler();
  }
  for (var = 0; var < SECTORS_COUNT; var++) {
      if (memcmp(buffer_write, (uint8_t*) (0x90000000 + var * MEMORY_SECTOR_SIZE), MEMORY_SECTOR_SIZE) != HAL_OK) {
          Error_Handler();
      }
  }
  ...
  // verify using indirect read
  for (var = 0; var < SECTORS_COUNT; var++) {
      if (CSP_QSPI_ReadMemory(buffer_read, var * MEMORY_SECTOR_SIZE, sizeof(buffer_read))) {
          Error_Handler();
      }
      if (memcmp(buffer_read, buffer_write, MEMORY_SECTOR_SIZE) != HAL_OK) {
          Error_Handler();
      }
  }
